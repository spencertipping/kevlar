...
kevlar = (function () {
  var original_kevlar = typeof kevlar === 'undefined' ? undefined : kevlar;
  var kevlar_global   = {};



// Kevlar transport-layer utilities | Spencer Tipping
// Licensed under the terms of the MIT source code license
...
// elsewhere, but I'm reimplementing stuff here for fun.)
...
//   });
  ...
  caterwaul.js_all()(function (require) {
    ...
    kevlar_global.server(endpoints, options) = handle_request
    ...
    -where [requests                      = {},
            ...
            default_options               = {},
            settings                      = caterwaul.merge(default_options, options) -effect [it.log_database || (it.log_database = kevlar_global.database('kevlar.server.log'))],
            ...
            invalid_parse_error(id, data) = log_request_error(id, {invalid: 'json',   data:   data})                -then- reply_with_error(id, 400)]})(require);
...
// For the moment I'm deferring indexing support. The priority at the moment is to get basic data storage going.
  ...
  caterwaul.js_all()(function (require) {
    kevlar_global.database(name) = ensure_directory_sync(name) -returning-
           ...
           log_generator_for(db)(table)         = ensure_directory_sync('#{db}/#{table}')
                                                  -returning- result -effect [it.find(bucket, each) = result -effect- read_bucket_contents(bucket, each)]
                                                      ...
                                                      -where [result(bucket, stuff)          = result -effect [append_to_bucket(bucket, JSON.stringify(stuff))],
                                                              ...
                                                              queues                         = {},
                                                              queue_for(bucket)              = queues[bucket] || (queues[bucket] = [])]]})(require);
